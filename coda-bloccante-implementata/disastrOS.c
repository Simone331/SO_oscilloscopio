...
void timerHandler(int j, siginfo_t *si, void *old_context) {
  if (running) {
    ...
    // nessun processo in esecuzione: entra direttamente nell’interrupt
    swapcontext(&main_context, &interrupt_context);
  }
}
...
void timerInterrupt(){
  int pid_out = running ? running->pid : -1;
  if (log_file)
    fprintf(log_file, "TIME: %d\tPID: %d\tACTION: %s\n",
            disastrOS_time, pid_out, "TIMER_OUT");

  ++disastrOS_time;
  // printf("time: %d\n", disastrOS_time); // opzionale

  // 1) SVEGLIA chi ha il timer scaduto
  TimerItem* t;
  while ((t = TimerList_current(&timer_list, disastrOS_time))) {
    PCB* p = t->pcb;
    TimerList_removeCurrent(&timer_list);
    p->timer = 0;  // azzera il puntatore al suo TimerItem

    // togli da waiting (se presente) e metti in ready
  ...
  // 2) SCHEDULING: se non c’è un running, prendine uno dai ready
  if (!running && ready_list.first) {
    PCB* next = (PCB*) List_detach(&ready_list, ready_list.first);
    next->status = Running;
    running = next;
  } else if (running && ready_list.first) {
    internal_schedule();
  }

  int pid_in = running ? running->pid : -1;
  if (log_file)
    fprintf(log_file, "TIME: %d\tPID: %d\tACTION: %s\n",
            disastrOS_time, pid_in, "TIMER_IN");

  if (running) {
    setcontext(&running->cpu_state);
  } else {
    // nessun processo pronto: restiamo nell’handler;
    // al prossimo tick riproveremo a svegliare qualcuno
  }
}
...
void disastrOS_start(void (*f)(void*), void* f_args, char* logfile){  
  ...
  syscall_vector[DSOS_CALL_BQ_CREATE] = internal_bq_create;
  syscall_numarg[DSOS_CALL_BQ_CREATE] = 2;
  syscall_vector[DSOS_CALL_BQ_DELETE] = internal_bq_delete;
  syscall_numarg[DSOS_CALL_BQ_DELETE] = 1;
  syscall_vector[DSOS_CALL_BQ_PUT] = internal_bq_put;
